...
-- Also ensure doctor cancel penalty trigger exists (see doctor_cancel_penalty migration).
...
-- Admins write via service role (admin API); optional authenticated admin update.
...
-- Turn on doctor cancel fines by default (admin can change amount).
