...
create table if not exists public.document_deliveries (
  ...
  recipient_type text not null default 'doctor'
    check (recipient_type in ('doctor')),
...
drop policy if exists admin_documents_portal_admin_all on public.admin_documents;
create policy admin_documents_portal_admin_all
...
drop policy if exists document_deliveries_portal_admin_all on public.document_deliveries;
create policy document_deliveries_portal_admin_all
...
drop policy if exists document_deliveries_doctor_select_own on public.document_deliveries;
create policy document_deliveries_doctor_select_own
  on public.document_deliveries
  for select
  to authenticated
  using (recipient_type = 'doctor' and recipient_id = auth.uid());

drop policy if exists document_deliveries_doctor_acknowledge_own on public.document_deliveries;
create policy document_deliveries_doctor_acknowledge_own
  on public.document_deliveries
  for update
  to authenticated
  using (recipient_type = 'doctor' and recipient_id = auth.uid())
  with check (recipient_type = 'doctor' and recipient_id = auth.uid());

drop policy if exists admin_documents_doctor_read_delivered on public.admin_documents;
create policy admin_documents_doctor_read_delivered
  on public.admin_documents
  for select
  to authenticated
  using (
    exists (
      ...
      where delivery.document_id = admin_documents.id
        and delivery.recipient_type = 'doctor'
        and delivery.recipient_id = auth.uid()
    )
  );

drop policy if exists "admin_documents_storage_admin_read" on storage.objects;
create policy "admin_documents_storage_admin_read" on storage.objects
  for select
  to authenticated
  using (bucket_id = 'admin-documents' and public.is_portal_admin());
...
as $$
  ...
  where delivery.recipient_type = 'doctor'
    and delivery.recipient_id = auth.uid()
...
drop policy if exists "admin_documents_storage_doctor_read" on storage.objects;
create policy "admin_documents_storage_doctor_read" on storage.objects
  for select
  to authenticated
  using (
